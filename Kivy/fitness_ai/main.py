...
class Setting(ModalView):
    ...
    def save_records(self, type):
        ...
        if type == "Save":
            db_engine.save("setting", data)
        
        else:
            db_engine.update("setting", data)
...
class Motivation(ModalView):
    ...
    def on_option(self, value, task):
        ...
        if value:
            ...
            if db_engine.read("motivation") is None:
                db_engine.save("motivation", data)

            else:
                db_engine.update("motivation", data) 
    
    def show(self):
        data = db_engine.read("motivation")
        if data is not None:
            for key, val in data.items():
                if val != "":
                    self._ignore = True
