	...
	def encrypt(cls, data):
		def _encrypt(data_str):
			if type(data_str) is str:
				return cls._public_key.encrypt( bytes( data_str, 'utf-8' ), padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()), algorithm=hashes.SHA256(), label=None)) #.decode("utf-8")
	...
	def decrypt(cls, data):
		def _decrypt(bytes_data):
			...
			if type(bytes_data) is bytes:
				return cls._private_key.decrypt( bytes_data , padding.OAEP(mgf=padding.MGF1(algorithm=hashes.SHA256()), algorithm=hashes.SHA256(),label=None) ).decode()
			else: return bytes_data
...
class DB():
	...
	def insert(self, tablename, row):
		conn = self._connect()
		if conn is None:
			print("db connection not available"); return False
		else:
			try:
				result 	= False
